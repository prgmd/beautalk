...
const weatherLine = computed(() => {
  ...
  const desc = weather.value.desc || wxMsg.value
  return `오늘 서울의 날씨는 ${weather.value.temp}°C ${desc}이네요.`
})
    ...
    <div class="main">
      <div class="page">
        ...
        <header class="greeting">
          ...
          <p v-if="weatherLine" class="weather-line">{{ weatherLine }}</p>
...
.weather-line { font-size: 13.5px; color: var(--ink-soft); margin-top: 6px; line-height: 1.55; }
.suggest { font-size: 13.5px; color: var(--ink-soft); margin-top: 4px; line-height: 1.55; }
...
@media (min-width: 900px) {
  ...
  .weather-line { font-size: 15px; margin-top: 8px; }
