...
- **추천 개수는 3개 고정.** 단, LLM이 존재하지 않는 id를 고르는 등으로 DB 조회 결과가 3개 미만이면
  **조회된 만큼만 반환**한다. 프론트는 개수에 무관하게 방어적으로 렌더한다.
- **실패 시**: §4-1과 동일 포맷(502/504 등). 프론트는 에러 안내 후 "다시 시도".

> 📌 **기피 성분 필터는 이번 범위에서 제외됐다.** 제품에 성분(ingredients) 데이터가 없어
> "기피 성분 포함 제품 제외"를 구현할 근거가 없기 때문. `SkinProfile.avoid_ingredients` 필드는
> 남겨두되 이번엔 사용하지 않는다. (성분 수집/필터는 추후 별도 합의 — §7 추후 항목)
...
- [x] **추천 개수 3개 고정.** DB 조회 실패로 부족할 때만 조회된 만큼(방어적 렌더).
- [x] **기피 성분 필터 = 이번 범위 제외.** 제품 성분 데이터 부재. `avoid_ingredients`는 미사용(필드는 유지).
...
### 추후(이번 범위 밖)
- [ ] **기피 성분 필터**: 크롤러에 성분(ingredients) 수집 추가 + `Product.ingredients` 필드 + 후처리 필터.
      `SkinProfile.avoid_ingredients`는 이미 있으므로 제품 쪽 데이터만 갖춰지면 붙일 수 있음.
- [ ] **사용량 제한 위치**: 현재 프론트에 로컬 usage 카운터 + 페이월 목업 존재.
      서버사이드(`ScopedRateThrottle`) 이전이 정석이나 이번 범위 밖 — **언제 이전할지**만 추후 합의.
- [ ] **PostgreSQL 전환**: 현재 dev는 SQLite. 배포/공유 DB 필요 시점에 `psycopg2` + 설정 전환.
